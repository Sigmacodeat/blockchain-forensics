"""zkSync Era Adapter - EVM-kompatibel (ZK Rollup)"""
...
class ZkSyncAdapter(EthereumAdapter):
    """zkSync Era blockchain adapter – EVM-kompatibel"""

    def __init__(self, rpc_url: Optional[str] = None):
        if rpc_url is None:
            try:
                from app.config import settings  # type: ignore
                rpc_url = getattr(settings, "ZKSYNC_RPC_URL", None)
            except Exception:
                rpc_url = None
        super().__init__(rpc_url or "mock://zksync")
        self._chain_name = "zksync"
        logger.info(f"Initialized zkSync adapter with RPC: {self.rpc_url}")
    ...
    def _determine_event_type(self, tx: Dict[str, Any], receipt: Dict[str, Any]) -> str:
        """zkSync-spezifische Bridge-Heuristiken (konfigurierbar) + Fallback."""
        try:
            from app.config import settings  # type: ignore
            cfg = getattr(settings, "BRIDGE_CONTRACTS_ZKSYNC", None)
        except Exception:
            cfg = None
