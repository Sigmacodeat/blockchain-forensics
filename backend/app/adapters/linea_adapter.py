"""Linea Adapter - EVM-kompatibel (ZK Rollup)"""
...
class LineaAdapter(EthereumAdapter):
    """Linea blockchain adapter – EVM-kompatibel"""

    def __init__(self, rpc_url: Optional[str] = None):
        if rpc_url is None:
            try:
                from app.config import settings  # type: ignore
                rpc_url = getattr(settings, "LINEA_RPC_URL", None)
            except Exception:
                rpc_url = None
        super().__init__(rpc_url or "mock://linea")
        self._chain_name = "linea"
        logger.info(f"Initialized Linea adapter with RPC: {self.rpc_url}")
    ...
    def _determine_event_type(self, tx: Dict[str, Any], receipt: Dict[str, Any]) -> str:
        """Linea-spezifische Bridge-Heuristiken (konfigurierbar) + Fallback."""
        try:
            from app.config import settings  # type: ignore
            cfg = getattr(settings, "BRIDGE_CONTRACTS_LINEA", None)
        except Exception:
            cfg = None
