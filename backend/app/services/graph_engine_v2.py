...
class GraphEngineV2:
    ...
    async def _get_neighbors(self, address: str, chain: str, constraints: PathConstraint) -> List[Dict[str, Any]]:
        ...
        try:
            def _run_query():
                with self._driver.session() as s:  # ensure session closes deterministically
    ...
    async def materialize_hot_paths(self, min_frequency: int = 100):
        ...
        try:
            def _run_hot_paths():
                with self._driver.session() as s:  # ensure session closes deterministically
                    return list(s.run(query, min_freq=min_frequency))
            records = await asyncio.to_thread(_run_hot_paths)
