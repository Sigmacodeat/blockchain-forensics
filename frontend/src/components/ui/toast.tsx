...
export const useToast = () => {
  ...
  if (context === undefined) {
    // Provide a no-op fallback for environments/tests without provider
    const noop = () => {}
    return {
      toasts: [],
      showToast: noop,
...
export const ToastProvider: React.FC<ToastProviderProps> = ({ children }) => {
  ...
  const showToast = useCallback((toast: Omit<Toast, 'id'>) => {
    ...
    const newToast: Toast = {
      id,
      duration: 5000, // Standard 5 Sekunden
      ...toast
    }
...
const ToastItem: React.FC<ToastItemProps> = ({ toast, onClose }) => {
  ...
  return (
    <div className={`p-4 rounded-lg border shadow-lg transition-all duration-300 ${getToastStyles()}`}>
...
// Convenience hooks für verschiedene Toast-Typen
...
export const useToastError = () => {
  ...
  return (title: string, message?: string, action?: Toast['action']) => {
    showToast({ type: 'error', title, message, action, duration: 0 }) // Error toasts bleiben länger
  }
}
