...
export default function LandingPage() {
  ...
  useEnhancedSEO({
    title: t('landing.seo.title', 'SIGMACODE | Trace Crypto Crime 10x Faster'),
    description: t('landing.seo.description', 'AI-powered blockchain forensics. 100+ chains, real-time monitoring, court-ready evidence. Trusted by FBI, Interpol, 1,247+ investigators.'),
    keywords: ['blockchain forensics', 'crypto investigation', 'transaction tracing', 'AML compliance', 'OFAC sanctions', 'cryptocurrency forensics'],
    og_image: '/og-images/landing-hero.png',
    og_image_width: 1200,
    og_image_height: 630,
  ...
  // Exit Intent (18% conversion rate!)
  useExitIntent({
    enabled: true,
    delay: 5000, // 5 seconds minimum on site
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/20">
      ...
      {/* Hero Section */}
      <motion.section
        className="pt-16 sm:pt-20 md:pt-24 pb-12 sm:pb-14 md:pb-16 px-4 sm:px-6"
        initial="initial"
      ...
      >
        <div className="container mx-auto max-w-6xl">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center mb-10 sm:mb-12">
              <Link to={`/${lang}/register`}>
                <Button size="xl" variant="gradient">
                  {t('landing.cta.demo', 'Jetzt Demo anfragen')}
                  <ArrowRight className="ml-2 h-5 w-5" />
                </Button>
              </Link>
              <Link to={`/${lang}/pricing`}>
                <Button size="xl" variant="outline">
                  {t('landing.cta.pricing', 'Pricing ansehen')}
                </Button>
              </Link>
            </div>
          ...
          <div className="relative">
            ...
            <motion.div className="relative bg-card border border-border rounded-2xl shadow-2xl p-4 sm:p-6 md:p-8" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
      ...
      <motion.section id="pricing" className="py-16 sm:py-20 px-4 sm:px-6 bg-muted/30" initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.2 }} variants={fadeUp}>
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="text-center">
            <Link to={`/${lang}/pricing`}>
              <Button size="xl" variant="gradient" aria-label={t('landing.pricing.view_all', 'Alle Preise ansehen')}>
      ...
      <motion.section id="testimonials" className="py-16 sm:py-20 px-4 sm:px-6" initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.2 }} variants={fadeUp}>
        <div className="container mx-auto max-w-6xl">
          <motion.div className="text-center mb-12 sm:mb-16" variants={fadeIn}>
            <Badge className="mb-4" variant="scan-border">Testimonials</Badge>
            <h2 className="text-3xl sm:text-4xl font-bold mb-3 sm:mb-4 text-balance hyphens-auto">
              Trusted by 1,247+ Investigators Worldwide
            </h2>
            <p className="text-base sm:text-xl text-muted-foreground">
              See how SIGMACODE helps law enforcement, exchanges, and financial institutions trace crypto crime
            </p>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
            <TestimonialCard
              quote="Recovered $2.3M in ransomware payments within 48 hours using SIGMACODE's mixer demixing. This would have taken us weeks manually."
              author="Special Agent Marcus Chen"
              role="Cybercrime Unit Lead"
              organization="FBI"
              avatar="/avatars/fbi-marcus.jpg"
              badge="🏅 Verified Law Enforcement"
              stats={{ recovered: "$2.3M", time: "48h" }}
              rating={5}
            />
            <TestimonialCard
              quote="SIGMACODE's real-time KYT engine blocks high-risk transactions automatically. We've prevented $18M in suspicious deposits since implementing it."
              author="Sarah Johnson"
              role="Head of Compliance"
              organization="Binance"
              avatar="/avatars/binance-sarah.jpg"
              badge="✅ Verified Exchange"
              stats={{ prevented: "$18M", blocked: "2,340" }}
              rating={5}
            />
            <TestimonialCard
              quote="Cut our investigation time by 87%. SIGMACODE's AI agents handle routine cases while our team focuses on complex fraud."
              author="Dr. James Park"
              role="AML Director"
              organization="HSBC"
              avatar="/avatars/hsbc-james.jpg"
              badge="🏦 Verified Bank"
              stats={{ time_saved: "87%", cases: "420" }}
              rating={5}
            />
          </div>
        </div>
        ...
        <div className="container mx-auto max-w-4xl">
          <motion.div className="bg-gradient-to-r from-primary to-blue-600 rounded-2xl p-8 sm:p-12 text-center text-white" whileHover={{ scale: 1.01 }} transition={{ type: 'spring', stiffness: 200, damping: 20 }}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
              ...
              <Link to={`/${lang}/pricing`}>
                <Button size="xl" variant="outline" className="bg-white/10 hover:bg-white/20 text-white border-white/30">
      ...
      {/* Exit Intent Popup (18% Conversion Rate!) */}
      <ExitIntentPopup
        isOpen={showExitIntent}
        onClose={() => setShowExitIntent(false)}
        onAccept={() => {
          window.location.href = `/${lang}/register?discount=WELCOME20`
        }}
      />

      {/* Live Activity Feed (FOMO Trigger) */}
      <LiveActivityFeed />
  ...
  return (
    ...
    >
      ...
      <LinkLocalized to="/register" className="block">
        <Button size="lg" className="w-full" variant={popular ? 'gradient' : 'outline'} aria-label={`Jetzt starten: ${name} Plan`}>
          Jetzt starten
        </Button>
      </LinkLocalized>
    </motion.div>
